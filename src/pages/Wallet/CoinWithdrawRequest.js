...
// import action
import { getCoinWithdrawRequest } from './../../actions/wallet';
...
const CoinWithdrawRequest = (props) => {
        ...
        { field: 'createdAt', headerName: 'Date', width: 200 },
        {
            ...
            renderCell: (params: CellParams) => {
                if (params.getValue('status') == 1) {
                    return (
                        <Button>{transactionStatus(params.getValue('status') == 1)}</Button>
                    )
    ...
    const fetchWithdrawRequest = async () => {
        try {
            const { status, loading, error, result } = await getCoinWithdrawRequest();
            if (status == 'success') {
                ...
                <Card>
                    <CardHeader color="primary">
                        <div className={classes.flexHeader}>
                            <h4 className={classes.cardTitleWhite}>Coin Withdraw Request List</h4>
                        </div>
                    </CardHeader>
                    <CardBody>
